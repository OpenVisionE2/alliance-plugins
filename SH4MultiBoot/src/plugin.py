...
class SH4MultiBootInstallation(Screen):
    ...
    def devpanel(self):
        try:
            ...
            self.session.open(HarddiskSetup)
        except:
...
class SH4MultiBootImageInstall(Screen, ConfigListScreen):
    ...
    def createSetup(self):
        ...
        self.list.append(getConfigListEntry(_('Copy current settings to the new image?'), self.sett))
    ...
    def imageInstall(self):
        ...
        if self.check_free_space():
            ...
            if myerror:
                ...
                message += "'"
                if fileExists(pluginpath + '/ex_init.pyo') or fileExists(pluginpath + '/ex_init.py'):
                ...
                print('[SH4MultiBoot]: ', cmd)
                self.session.open(Console, _('SH4MultiBoot: Install new image'), [message, cmd])
...
def Plugins(**kwargs):
    return [PluginDescriptor(name='SH4MultiBoot', description='SH4 Multi Boot', where=PluginDescriptor.WHERE_MENU, fnc=menu), PluginDescriptor(name='SH4MultiBoot', description=_('E2 light MultiBoot for SH4'), icon='plugin.png', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)]
